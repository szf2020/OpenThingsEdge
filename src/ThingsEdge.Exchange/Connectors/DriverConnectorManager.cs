...
/// 设备驱动连管理接器。
/// </summary>
internal sealed class DriverConnectorManager(
    IOptions<ExchangeOptions> options,
    ILogger<DriverConnectorManager> logger) : IDriverConnectorManager
{
    private readonly ConcurrentDictionary<string, IDriverConnector> _connectors = [];
    ...
    public async Task<IDriverConnector> CreateAndConnectAsync(Device deviceInfo, CancellationToken cancellationToken = default)
    {
        ...
        DriverConnector connector = new(deviceInfo.DeviceId, deviceInfo.Host, driverNet.Port, driverNet);
        ...
        // 将连接添加缓存中
        _connectors.AddOrUpdate(deviceInfo.DeviceId, connector, (_, _) => connector);

        return connector;
    }

    public IDriverConnector? GetConnector(string id)
    {
    ...
    {
        _connectors.TryRemove(id, out var _);
    }
}
