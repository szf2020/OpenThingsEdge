...
namespace ThingsEdge.Exchange.Connectors.Drivers;
...
{
    extension(SiemensS7Net siemensS7Net)
    {
        /// <summary>
        /// S7 协议批量读取。
        /// </summary>
        /// <param name="tags">要批量读取的标记集合</param>
        public async Task<(bool ok, List<PayloadData>? data, string? err)> ReadMultiAsync(IEnumerable<Tag> tags)
        {
            // 取最短的长度。
        ...
        private async Task<(bool ok, List<PayloadData>? data, string? err)> ReadPartMultiAsync(IEnumerable<Tag> tags)
        {
            var count = tags.Count();

            List<string> addresses = [with(count)];
            List<ushort> lengths = [with(count)];
            foreach (var tag in tags)
            {
            ...
            var index = 0;
            List<PayloadData> list = [with(count)];
            foreach (var tag in tags)
            {
