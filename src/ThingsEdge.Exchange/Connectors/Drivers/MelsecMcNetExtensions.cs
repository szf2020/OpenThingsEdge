...
/// </summary>
internal static class MelsecMcNetExtensions
{
    extension(MelsecMcNet melsecMcNet)
    {
        /// <summary>
        /// 三菱 MC 协议读取连续的地址。
        /// </summary>
        ...
        public async Task<(bool ok, List<PayloadData>? data, string? err)> ReadContinuationAsync(IEnumerable<Tag> tags)
        {
            ushort length = 0;
            ...
            var index = 0;
            List<PayloadData> list = [with(tags.Count())];
            foreach (var tag in tags)
            {
